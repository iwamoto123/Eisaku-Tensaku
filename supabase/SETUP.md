...
## 3.6. メールのテンプレートを直す（推奨）

初期状態のリンクは、**ログインを始めたブラウザと同じブラウザで開かないと失敗します。**
パソコンでログイン操作をして、リンクをスマホで開くと通りません。
テンプレートを次のように変えると、どの端末で開いても通るようになります。

1. 左メニューの **Authentication** → **Emails**（Email Templates）を開く
2. **Magic Link** を選ぶ
3. 本文のリンク部分を、次の形に差し替える

```html
<h2>ログイン</h2>
<p>下のリンクを押すと、英作文添削メーカーにログインできます。</p>
<p><a href="{{ .SiteURL }}/auth/callback?token_hash={{ .TokenHash }}&type=magiclink">ログインする</a></p>
<p>心当たりがない場合は、このメールを無視してください。</p>
```

`{{ .ConfirmationURL }}` を使わず `{{ .TokenHash }}` を使うのがポイントです。
...
登録した人は、アプリのログイン画面にメールアドレスを入れるとリンクが届き、それを押すだけでログインできます。

**パスワードでもログインできます。** ログイン画面の「パスワードでログインする」から入れます。
Supabaseの標準メール送信は**1時間に2通まで**という厳しい上限があるため、
動作確認のときや、上限に当たったときはこちらを使ってください。

手順3で設定したパスワードがそのまま使えます。忘れた場合は
**Authentication → Users** の対象ユーザーの「…」から再設定できます。

### 講師が増えてきたら

1時間2通の上限は、講師4〜5人の運用には足りません。
配布する前に、独自のSMTP（Resend など）を設定して上限を上げてください。
